...
try{
  ...
  message.author.username = message.author.username.replace(regax,"");
  if (message.author.bot) return;
 const now = new Date();
  const insertTime = `${now.getFullYear()}-${Number(now.getMonth())+1}-${now.getDate()} ${now.getHours()}:${now.getMinutes()}:${now.getSeconds()}`
  connection.query(
    `insert into BotChat (server,channel,name,contents,time) values ("${encodeURI(message.channel.guild.name)}","${encodeURI(message.channel.name)}","${encodeURI(message.author.username)}","${encodeURI(message.content)}","${insertTime}");`
     );
