...
async function sendLog(message,number,insertTime){
    try{
        ...
        if(number == "1")
        {
           ...
           await fs.writeFileSync('/home/remaylin/newerr.txt',`${insertTime}`,function(err){if(err === null)console.log("에러로그 정상출력")})
        }
        if(number == "2")
        {
           await message.reply({files:['/home/remaylin/newoutput.txt'] });
           await fs.unlinkSync('/home/remaylin/newoutput.txt')
           await fs.writeFileSync('/home/remaylin/newoutput.txt',`${insertTime}`,function(err){if(err === null)console.log("봇로그 정상출력")})
        }
